...
vec2 Bezier(float time)
{
	...
	return invTime*invTime*(StartPos*invTime + 3*Control1*time) + (3*Control2*invTime + EndPos*time)*time*time;
}
...
float Bezier4SignedDistance(vec2 uv)
{
...
void main()
{
	...
	float signedDistance = 1.0f - (Bezier4SignedDistance(fTexCoord1) / (Thickness/2));
